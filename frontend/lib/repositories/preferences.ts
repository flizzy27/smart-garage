...
export async function findPreferencesForUser(userId: string): Promise<UserSettings> {
  ...
  return {
    ...
    distanceUnit: sanitizeDistanceUnit(row.distanceUnit),
    volumeUnit: sanitizeVolumeUnit(row.volumeUnit),
    hiddenVehicleFields: sanitizeHiddenVehicleFields(row.hiddenVehicleFields),
  ...
  return withDbRetry(() =>
    prisma.userPreferences.upsert({
      ...
      create: {
        ...
        hiddenVehicleFields: serializeHiddenVehicleFields(settings.hiddenVehicleFields),
