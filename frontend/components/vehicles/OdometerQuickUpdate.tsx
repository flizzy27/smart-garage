    ...
    return (
      ...
      >
        <span className="tabular-nums font-semibold text-foreground">
          {formatDistance(currentKm, locale, distanceUnit)}
        </span>
    ...
    <div className={compact ? "mt-2" : ""}>
      ...
      <form action={action} className="flex flex-wrap items-end gap-2">
        ...
        <div className="space-y-1">
          <Label htmlFor={`odometer-${vehicleId}`}>
            {t("label")} ({distanceUnitLabel(distanceUnit)})
          </Label>
          <Input
            ...
            min={0}
            required
            defaultValue={formDistanceValue(currentKm, distanceUnit)}
            className="w-36"
          />
        </div>
